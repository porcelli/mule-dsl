...
public class FlowOutboundExamples {
    ...
    public static class FlowOutbounds extends AbstractModule {
        ...
        public void configure() {
            EndpointProcessor out = defineEndpoint("ex_outbound").using(VM.path("internal"));
            ...
            newFlow("MyFlow").process(
                    ...
                    send(out),
                    //protocol specific
                    send(SMTPS.user("${user}")
                            .password("${password}")
                            .host("${host}")
                            .from("${from}")
                            .subject("Your order has been placed!")),
                    //protocol specific
                    send(VM.path("outQueue")),
                    ...
                    sendAndWait(SMTPS.user("${user}")
                            .password("${password}")
                            .host("${host}")
                            .from("${from}")
                            .subject("Your order has been placed!")),
                    //protocol specific - sync
                    sendAndWait(VM.path("outQueue"))
            );
        }
