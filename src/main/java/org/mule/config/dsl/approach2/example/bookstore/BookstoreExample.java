 ...
 * LICENSE.txt file.
 */
package org.mule.config.dsl.approach2.example.bookstore;

import org.mule.config.dsl.approach2.AbstractModule;
import org.mule.config.dsl.approach2.example.bookstore.business.*;
...
public class BookstoreExample {
    ...
    public static class BookStore extends AbstractModule {
        ...
        public void configure() {
            ...
            defineEndpoint("stats").using(VM.path("statistics"));

            Transformer setHtmlContentType = newTransformer("setHtmlContentType")
            ...
            newFlow("CatalogService").in(
                    //Public interface
                    from(HTTP.listen(host("0.0.0.0").port(8777).path("services/catalog"))
                            .as(WS.with(CatalogService.class))),
            ...
            newFlow("OrderService").in(
                    //Public interface
                    from(HTTP.listen(host("0.0.0.0").port(8777).path("services/order"))
                            .as(WS.with(OrderService.class)))
            ).process(
                    execute(OrderServiceImpl.class).asSingleton(),
                    send(VM.path("emailNotification")),
                    send(VM.path("dataWarehouse"))
            );


            newFlow("EmailNotificationService").in(
                    from(VM.path("emailNotification"))
            ).process(
